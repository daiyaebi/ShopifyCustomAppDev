...
exports.Project = exports.InstallMode = void 0;
...
(function (InstallMode) {
     ...
     * an updated lockfile.
     */
    InstallMode["UpdateLockfile"] = "update-lockfile";
    /**
     * Don't run the build scripts.
     */
    InstallMode["SkipBuild"] = "skip-build";
...
const makeLockfileChecksum = (normalizedContent) => hashUtils.makeHash(`${INSTALL_STATE_VERSION}`, normalizedContent);
class Project {
        ...
        if (fslib_2.xfs.existsSync(lockfilePath)) {
            ...
            if (parsed.__metadata) {
                ...
                for (const key of Object.keys(parsed)) {
                    ...
                    const conditions = (_a = data.conditions) !== null && _a !== void 0 ? _a : null;
    ...
    async addWorkspace(workspaceCwd) {
        ...
        if (typeof dup !== `undefined`)
            throw new Error(`Duplicate workspace name ${structUtils.prettyIdent(this.configuration, workspace.locator)}: ${fslib_1.npath.fromPortablePath(workspaceCwd)} conflicts with ${fslib_1.npath.fromPortablePath(dup.cwd)}`);
    ...
    tryWorkspaceByCwd(workspaceCwd) {
        ...
        workspaceCwd = fslib_2.ppath.normalize(workspaceCwd)
            .replace(/\/+$/, ``);
    ...
    refreshWorkspaceDependencies() {
        for (const workspace of this.workspaces) {
            ...
            if (!pkg)
                throw new Error(`Assertion failed: Expected workspace ${structUtils.prettyWorkspace(this.configuration, workspace)} (${formatUtils.pretty(this.configuration, fslib_2.ppath.join(workspace.cwd, fslib_2.Filename.manifest), formatUtils.Type.PATH)}) to have been resolved. Run "yarn install" to update the lockfile`);
    ...
    async resolveEverything(opts) {
        ...
        const resolverChain = opts.lockfileOnly
            ? [new RunInstallPleaseResolver_1.RunInstallPleaseResolver(realResolver)]
        ...
        await opts.report.startProgressPromise(Report_1.Report.progressViaTitle(), async (progress) => {
            ...
            const startDescriptorResolution = async (descriptor) => {
                ...
                const resolvedDependencies = new Map(await miscUtils.allSettledSafe(resolutionDependenciesList.map(async (dependency) => {
                    ...
                    return [dependency.descriptorHash, resolvedPackage];
                })));
        ...
        for (const pkg of allPackages.values()) {
            ...
            if (!structUtils.isPackageCompatible(pkg, supportedArchitectures)) {
                if (structUtils.isPackageCompatible(pkg, currentArchitecture)) {
                    opts.report.reportWarningOnce(MessageName_1.MessageName.GHOST_ARCHITECTURE, `${structUtils.prettyLocator(this.configuration, pkg)}: Your current architecture (${process.platform}-${process.arch}) is supported by this package, but is missing from the ${formatUtils.pretty(this.configuration, `supportedArchitectures`, Configuration_1.FormatType.SETTING)} setting`);
                }
    ...
    async fetchEverything({ cache, report, fetcher: userFetcher, mode }) {
        const cacheOptions = {
        ...
        const limit = (0, p_limit_1.default)(FETCHER_CONCURRENCY);
    ...
    async linkEverything({ cache, report, fetcher: optFetcher, mode }) {
        ...
        const fetcherOptions = { checksums: this.storedChecksums, project: this, cache, fetcher, report, skipIntegrityCheck: true, cacheOptions };
        const linkers = this.configuration.getLinkers();
        const linkerOptions = { project: this, report };
        ...
        for (const locatorHash of this.accessibleLocators) {
            ...
            if (workspace !== null) {
                ...
                try {
                    ...
                    if (holdPromises.length === 0) {
                        (_b = fetchResult.releaseFs) === null || _b === void 0 ? void 0 : _b.call(fetchResult);
                    }
                    else {
        ...
        await opts.report.startTimerPromise(`Post-resolution validation`, {
            ...
            if (initialLockfile !== null) {
                ...
                if (newLockfile !== initialLockfile) {
                    const diff = (0, diff_1.structuredPatch)(lockfilePath, lockfilePath, initialLockfile, newLockfile);
        ...
        for (const entry of await fslib_2.xfs.readdirPromise(cache.cwd)) {
            ...
            if (cache.immutable) {
                ...
                if (preferAggregateCacheInfo)
                    ...
                    report.reportInfo(MessageName_1.MessageName.UNUSED_CACHE_ENTRY, `${formatUtils.pretty(this.configuration, fslib_2.ppath.basename(entryPath), `magenta`)} appears to be unused - removing`);
                await fslib_2.xfs.removePromise(entryPath);
            }
        }
        if (preferAggregateCacheInfo && entriesRemoved !== 0) {
            report.reportInfo(MessageName_1.MessageName.UNUSED_CACHE_ENTRY, entriesRemoved > 1
...
function applyVirtualResolutionMutations({ project, allDescriptors, allResolutions, allPackages, accessibleLocators = new Set(), optionalBuilds = new Set(), peerRequirements = new Map(), volatileDescriptors = new Set(), report, tolerateMissingPackages = false, }) {
    ...
    const reportStackOverflow = () => {
        ...
        fslib_2.xfs.detachTemp(logDir);
        throw new Report_1.ReportError(MessageName_1.MessageName.STACK_OVERFLOW_RESOLUTION, `Encountered a stack overflow when resolving peer dependencies; cf ${fslib_1.npath.fromPortablePath(logFile)}`);
    };
    ...
    const resolvePeerDependencies = (parentDescriptor, parentLocator, peerSlots, { top, optional }) => {
    ...
    const resolvePeerDependenciesImpl = (parentDescriptor, parentLocator, peerSlots, { top, optional }) => {
        ...
        for (const warning of miscUtils.sortMap(warnings, warningSortCriterias)) {
            switch (warning.type) {
                case WarningType.NotProvided:
                    {
                        report.reportWarning(MessageName_1.MessageName.MISSING_PEER_DEPENDENCY, `${structUtils.prettyLocator(project.configuration, warning.subject)} doesn't provide ${structUtils.prettyIdent(project.configuration, warning.requested)} (${formatUtils.pretty(project.configuration, warning.hash, formatUtils.Type.CODE)}), requested by ${structUtils.prettyIdent(project.configuration, warning.requester)}`);
                    }
                    break;
